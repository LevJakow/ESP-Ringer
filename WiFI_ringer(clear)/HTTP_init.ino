...
void HTTP_init(void) {
  ...
  // API для устройства
  HTTP.on("/ssdp", handle_Set_Ssdp);     // Установить имя SSDP устройства по запросу вида /ssdp?ssdp=Ringer
  HTTP.on("/ssid", handle_Set_Ssid);     // Установить имя и пароль роутера по запросу вида /ssid?ssid=home2&password=12345678
  HTTP.on("/ssidap", handle_Set_Ssidap); // Установить имя и пароль для точки доступа по запросу вида /ssidap?ssidAP=home1&passwordAP=12345678
  ...
  HTTP.on("/user1set", handle_user1); //Путь к изображению USER1
  HTTP.on("/user2set", handle_user2); //Путь к изображению USER2
  HTTP.on("/user3set", handle_user3); //Путь к изображению USER3
  HTTP.on("/keys", handle_keys); //назначение звуковых файлов для основных кнопок
  HTTP.on("/logo", handle_logo); //путь к логотипу компании
  HTTP.on("/play", play_user);  //проиграть звук N по запросу типа  http://192.168.0.101/play?play=N
  HTTP.on("/volume", setvolume);  //установка громкости mp3 модуля по запросу типа  http://192.168.0.101/volume?volume=30
  HTTP.on("/rf", last_rf); //функция возвращает код последнего принятого сигнала
  HTTP.on("/rfset", handle_rf); //назначение кодов для управления с пульта 433
  HTTP.on("/clientip",setclientip);  //установка ip клиента пульта управления по wifi
  ...
  HTTP.on("/mode",handle_mode); //настройка режимов работы колонки
  // Запускаем HTTP сервер
  HTTP.begin();
}
void handle_mode() { //установка ip клиента
  modes._blynk = HTTP.arg("tblynk").toInt(); 
  modes._mdns = HTTP.arg("tmdns").toInt();     
  modes._start = HTTP.arg("tstart").toInt();
  modes._stop = HTTP.arg("tstop").toInt();
  modes._days = HTTP.arg("tdays").toInt();
  modes._hello = HTTP.arg("thello").toInt();
  modes._history = HTTP.arg("thistory").toInt();
  modes._light = HTTP.arg("tlight").toInt();
  saveConfig();           
